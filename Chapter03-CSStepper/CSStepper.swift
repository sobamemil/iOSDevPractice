...
class CSStepper: UIControl {
    ...
    public var value: Int = 0 {
        didSet { // 프로퍼티의 값이 바뀌면 자동으로 호출
            ...
            // 이 클래스를 사용하는 객체들에게 valueChanged 이벤트 신호를 보냄
    ...
    // 센터 영역의 색상
    @IBInspectable
    public var bgColor: UIColor = UIColor.red {
        didSet {
...
//        self.rightBtn.setTitle("⬆", for: .normal) // 버튼의 타이틀
        self.rightBtn.setTitle(rightTitle, for: .normal) // 버튼의 타이틀
...
//        self.centerLabel.backgroundColor = .red // 배경 색상 지정
    ...
    @objc public func valueChange(_ sender: UIButton) {
        ...
        // 스테퍼의 값을 변경하기 전에, 미리 최소값과 최대값의 범위를 벗어나지 않는지 체크
