...
class RevealViewController: UIViewController {
    ...
    func getSideView() {
        ...
        // 사이드바 컨트롤러에 부모 뷰 컨트롤러가 바뀌었음을 알려줌
        vc.didMove(toParent: self)
    ...
    func setShadowEffect(shadow: Bool, offset: CGFloat) {
        if (shadow == true) { // 그림자 효과 설정
            ...
            self.contentVC?.view.layer.shadowColor = UIColor.white.cgColor // 그림자 색상
    ...
    func openSideBar(_ complete: ( () -> Void)? ) {
        ...
        UIView.animate(withDuration: TimeInterval(self.SLIDE_TIME), delay: TimeInterval(0), options: options, animations: {
            self.contentVC?.view.frame = CGRect(x: self.SIDEBAR_WIDTH, y: 0, width: self.view.frame.size.width, height: self.view.frame.height)
        }, completion: {
            if $0 == true {
    ...
    func closeSideBar(_ complete: ( () -> Void)? ) {
        ...
        UIView.animate(withDuration: TimeInterval(self.SLIDE_TIME), delay: TimeInterval(0), options: options, animations: {
            ...
            self.contentVC?.view.frame = CGRect(x: 0, y: 0, width: self.view.frame.size.width, height: self.view.frame.size.height)
        }, completion: {
            if $0 == true {
                // 사이드 바 뷰를 제거
